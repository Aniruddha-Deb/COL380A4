...
def compare_blocks(correct_block, your_block, m, diag):
    ...
    if diag:
        correct_mat = np.frombuffer(correct_block, dtype=eval(f"np.uint32")).reshape((m, m))
        your_mat = np.frombuffer(your_block, dtype=eval(f"np.uint32")).reshape((m, m))
...
def compare_files(standard_output, submission_output):
    ...
    if n1 != n2 or m1 != m2:
        ...
        return 0, log
    elif file_size != 12 + k2 * (8 + 4 * m2**2):
        log = f"file_size({file_size}) != 12 + k2 * (8 + {4} * m**2) = {12 + k2 * (8 + 4 * m2**2)}"
        return 0, log
    n, m = n1, m1
    ...
    for x in correct_index:
        if x in your_index:
            ...
            correct_block = f1.read(4 * m**2)
            your_block = f2.read(4 * m**2)
...
if __name__ == "__main__":
    ...
    for i in range(1,n_tests+1):
        print(f'Test {i}:')
        output = subprocess.run(['time', './a4', f'data/test{i}/A.mat', f'data/test{i}/B.mat', f'data/test{i}/C_pred.mat'], capture_output=True)
        realtime = str(output.stderr).split(' ')[2]
        print("    Time: ", realtime)
        acc, verdict = compare_files(f'data/test{i}/C_pred.mat', f'data/test{i}/C.mat')
        print("   ", acc, verdict)
        subprocess.run(['rm', f'data/test{i}/C_pred.mat'])
